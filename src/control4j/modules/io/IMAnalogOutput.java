...
/**
 *  Sets outputs of the analog output hardware.
 */
public class IMAnalogOutput extends InputModule
{

  /**
   *  Analog output resource.
   */
  @Resource
  ...
  protected void initialize(IConfigBuffer configuration)
  {
    ...
    if (getNumberOfAssignedInputs() > hardware.getNumberOfAnalogOutputs())
  ...
  public void put(Signal[] input)
  {
    ...
    for (int i=0; i<size; i++)
      if (input[i] != null && input[i].isValid())
        hardware.setAnalogOutput(i, input[i].getValue());
  }
}
